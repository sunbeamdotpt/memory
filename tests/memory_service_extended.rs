...
async fn test_switch_model_no_facts() {
    ...
    memory.switch_model(EmbeddingModelType::CodeBert).await.unwrap();
    let model = memory.current_model().await;
...
async fn test_list_facts_date_filtering() {
    ...
    let results = memory.list_facts("dated", 10, Some(3000000000), Some(4000000000)).await.unwrap();
    assert!(results.is_empty());

    // Filter with a past range → no results
    let results = memory.list_facts("dated", 10, Some(0), Some(1000)).await.unwrap();
    assert!(results.is_empty());
}
...
async fn test_update_fact_preserves_namespace() {
    ...
    let fact = memory.add_fact("original-ns", "original content", None).await.unwrap();
    assert_eq!(fact.namespace, "original-ns");

    let updated = memory.update_fact(&fact.id, "updated content", None).await.unwrap();
...
async fn test_update_fact_with_source() {
    ...
    let updated = memory.update_fact(
        &fact.id,
        "new content",
        Some("urn:smem:code:fs:/home/user/file.rs")
    ).await.unwrap();
    assert_eq!(updated.source, Some("urn:smem:code:fs:/home/user/file.rs".to_string()));
}
...
async fn test_search_facts_namespace_filter() {
    ...
    memory.add_fact("animals", "elephants are big", None).await.unwrap();
    memory.add_fact("plants", "trees are tall", None).await.unwrap();

    let results = memory.search_facts("elephants", 5, Some("animals")).await.unwrap();
    assert!(!results.is_empty());
...
async fn test_semantic_store_rebuild_vectors_empty() {
    ...
    let count = store.rebuild_vectors(768, |_texts| {
        Ok(vec![vec![0.0; 768]])
    }).await.unwrap();
    assert_eq!(count, 0);
}
...
async fn test_list_facts_with_source() {
    let (memory, _dir) = setup().await;
    memory.add_fact("docs", "doc content", Some("urn:smem:doc:fs:/home/user/file.md")).await.unwrap();
    let results = memory.list_facts("docs", 10, None, None).await.unwrap();
    assert_eq!(results.len(), 1);
    assert_eq!(results[0].source, Some("urn:smem:doc:fs:/home/user/file.md".to_string()));
}
