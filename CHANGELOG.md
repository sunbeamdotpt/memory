...
- Hardened FTS5 keyword-search sanitization so colons and other FTS5 query-syntax characters (e.g. `:`, `*`, `"`, `+`, `-`, `(`, `)`) are stripped and every remaining token is quoted. This fixes "Database error: no such column: …" failures on queries such as `kms: vault ...`.
- The file watcher and ingestion pipeline now ignore paths inside `.git` directories (including transient files like `.git/index.lock`), preventing ingestion errors while scanning watched repositories.
